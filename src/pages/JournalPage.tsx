...
import { Loader2, Book, Trash2, ChevronLeft, ChevronRight, PlusCircle, PenSquare, Trophy } from 'lucide-react';
...
const JournalPage: React.FC = () => {
    ...
    const handleLogDetox = async (values: z.infer<typeof detoxFormSchema>) => {
        const points = Math.round(values.duration / 6); // 10 points per hour
        try {
            ...
            toast.error("Failed to log detox time.");
        }
    };
        ...
        <div className="space-y-4 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-9">
                    <div 
                        className="relative grid grid-cols-1 md:grid-cols-2 shadow-2xl rounded-lg overflow-hidden border-4 border-primary bg-primary w-full"
                        style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.4), inset 0 0 10px rgba(0,0,0,0.3)' }}
                    >
                        ...
                        <div className="bg-card p-6 md:p-8 flex flex-col min-h-[600px]" style={{backgroundImage: "url('/paper-texture.png')"}}>
                            ...
                            <div className="flex-1 overflow-y-auto -mr-2 pr-2 space-y-1 font-serif">
                                {entries.map((entry, index) => (
                                    <button 
                                        key={entry.id}
                                        onClick={() => { setActiveIndex(index); setIsWriting(false); }}
                                        className={cn("w-full text-left p-2 rounded-md text-sm transition-colors", activeIndex === index && !isWriting ? "bg-primary/10 text-primary font-bold" : "hover:bg-primary/5")}
                                    >
                            ...
                            <Button variant="outline" className="w-full mt-4 border-primary/30" onClick={() => setIsWriting(true)}>
                        ...
                        <div className="bg-card p-6 md:p-8 flex flex-col min-h-[600px]" style={{backgroundImage: "url('/paper-texture.png')"}}>
                            {isWriting ? (
                                <>
                                    <h3 className="font-display text-2xl font-bold mb-4 border-b-2 border-primary/20 pb-2">New Page</h3>
                                    <Form {...journalForm}>
                                        <form onSubmit={journalForm.handleSubmit(handleAddEntry)} className="flex flex-col flex-1">
                                            <FormField control={journalForm.control} name="content" render={({ field }) => (
                                                <FormItem className="flex-1 flex flex-col">
                                                    <FormControl><Textarea placeholder={journalPrompts} className="flex-1 font-serif text-base resize-none bg-transparent border-none focus-visible:ring-0 focus-visible:ring-offset-0 p-0" {...field} /></FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )}/>
                                            <div className="flex justify-end gap-2 mt-4">
                                                {entries.length > 0 && <Button type="button" variant="ghost" onClick={() => setIsWriting(false)}>Cancel</Button>}
                                                <Button type="submit" disabled={journalForm.formState.isSubmitting}>
                                                    {journalForm.formState.isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}
                                                    Inscribe Entry
                                                </Button>
                                            </div>
                                        </form>
                                ...
                                <div className="flex-1 flex flex-col font-serif">
                                    <div className="flex items-center justify-between border-b-2 border-primary/20 pb-2 mb-4">
                                        <h3 className="font-display text-xl font-bold">{new Date(activeEntry.entry_date + 'T00:00:00').toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</h3>
                                        <AlertDialog>
                                            ...
                                            <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Remove this page?</AlertDialogTitle><AlertDialogDescription>This action cannot be undone.</AlertDialogDescription></AlertDialogHeader>
                                                <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => handleDeleteEntry(activeEntry.id)} className="bg-destructive hover:bg-destructive/90">Remove Page</AlertDialogAction></AlertDialogFooter>
                                            </AlertDialogContent>
                                        </AlertDialog>
                                    </div>
                                    <div className="prose prose-sm max-w-none flex-1 overflow-y-auto prose-p:text-base prose-headings:font-display">
                                        <ReactMarkdown>{activeEntry.content}</ReactMarkdown>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex flex-col items-center justify-center flex-1 text-center text-muted-foreground font-serif">
                                    <Book className="h-16 w-16 mb-4"/>
                                    <p>Select an entry from the Contents page to read.</p>
                                </div>
                            )}
                        </div>
                    </div>
                    <div className="flex items-center justify-center mt-4 gap-4">
                        ...
                        <span className="text-sm text-muted-foreground">{entries.length > 0 && !isWriting ? `Page ${activeIndex + 1} of ${entries.length}` : ''}</span>
                ...
                {/* --- Digital Detox Sidebar --- */}
                <div className="lg:col-span-3">
                    <Card className="sticky top-24 animate-slide-up-fade" style={{animationDelay: '0.2s'}}>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2"><Trophy className="h-5 w-5 text-accent" />Clarity Rewards</CardTitle>
                            <CardDescription>Earn points for time spent away from digital distractions.</CardDescription>
                        </CardHeader>
                        <CardContent className="flex flex-col items-center justify-center space-y-4 text-center">
                            <div>
                                <p className="text-5xl font-bold">{profile?.clarity_points ?? 0}</p>
                                <p className="text-sm text-muted-foreground">Total Clarity Points</p>
                            </div>
                            <Dialog open={isDetoxModalOpen} onOpenChange={setIsDetoxModalOpen}>
                                ...
                                <DialogContent className="bg-card/80 backdrop-blur-sm">
                                    ...
                                    <Form {...detoxForm}>
                                        <form onSubmit={detoxForm.handleSubmit(handleLogDetox)} className="space-y-4 pt-4">
